...
- [ ] ខ្ញុំអាចហៅឈ្មោះភាពរញ៉េរញ៉ៃពិតដែល version control ការពារ (zips, “final-final”, ការផ្លាស់ប្តូរបាត់)
- [ ] ខ្ញុំអាចពន្យល់ Git vs GitHub ក្នុងមួយប្រយោគៗ
...
- [ ] ខ្ញុំអាចផ្គូផ្គង working tree → staging → commit → history ទៅរឿងវ៉ាលី
- [ ] ខ្ញុំដឹងថាហេតុអ្វី staging មាន (commit តែអ្វីដែលសមនឹងគ្នា)
...
- [ ] ខ្ញុំដឹងថាពេលណាត្រូវប្រើ `git add FILE` ជំនួស `git add .`
...
- Pull / merge ជាញឹកញាប់ ដូច្នេះ conflict នៅតូច
...
| branch ចែករំលែក | សុវត្ថិភាពជាងមុន | ប្រថុយប្រសិនបើអ្នកផ្សេងទាញ commit របស់អ្នករួចហើយ |
| ការប្រើប្រាស់ធម្មតា | feature merge → `main` | ធ្វើបច្ចុប្បន្នភាពលក្ខណៈពិសេស *របស់អ្នក* ទៅកាន់ `main` |

### ច្បាប់មាស (ចងចាំនេះ)

**កុំធ្វើបាបអ្នកដ៏ទៃដែលបានបង្កើតរួចហើយ** — ជាពិសេសចែករំលែក `main`។

បង្កើត branch feature *local* របស់អ្នកឡើងវិញ។ ចូលចិត្តការ merge (ឬត្រឡប់) សម្រាប់ប្រវត្តិដែលមនុស្សគ្រប់គ្នាចែករំលែក។
...
### “សារចុងក្រោយគឺខុស” (មិនទាន់បាន push)

```bash
git commit --amend -m "Better message"
```

### "ខ្ញុំភ្លេចឯកសារមួយនៅក្នុង commit ចុងក្រោយ" (មិនត្រូវបាន push)
...
### "បោះបង់ commit ចុងក្រោយ រក្សាការងាររបស់ខ្ញុំ"
...
### "លុបអ្វីគ្រប់យ៉ាងត្រលប់ទៅ commit ចុងក្រោយ" (គ្រោះថ្នាក់)
...
### “យើងបាន push រួចហើយ — កុំសរសេរប្រវត្តិសាស្ត្រឡើងវិញ”
...
1. បង្កើត repo ទទេនៅលើ GitHub (រំលង README ប្រសិនបើអ្នកមាន commit ក្នុងតំបន់រួចហើយ)
2. ភ្ជាប់ និង push៖
...
`-u` ចងចាំ upstream ដូច្នេះក្រោយមក `git push` / `git pull` ខ្លីជាង។
...
`origin/main` គឺជា *រូបភាព* របស់កុំព្យូទ័រយួរដៃរបស់អ្នក `main` របស់ GitHub។ `fetch` ធ្វើបច្ចុប្បន្នភាពរូបភាព; `pull` នាំយក commit ទាំងនោះទៅក្នុង branch របស់អ្នក។
...
អ្នកបានជួសជុលកំហុសក្នុងការចូល។ អ្នកមិនគួរ**មិន**push ត្រង់ទៅ `main` នៅលើក្រុមមួយ។**សំណើរទាញ (PR)**និយាយថា៖ "សូមពិនិត្យមើល branch នេះ បន្ទាប់មកបញ្ចូលវាចូលគ្នា។"
...
មនុស្សប្រាំនាក់ pushcommit ដោយចៃដន្យទៅកាន់ `main`។ ដាក់ពង្រាយការបំបែក។ គ្មាននរណាដឹងថាបានដឹកជញ្ជូនអ្វីទេ។ លំហូរការងារមាន ដូច្នេះក្រុមការងាររក្សាបានលឿន * និង * មានសុវត្ថិភាព។

### ទម្លាប់ដែលមានមាត្រដ្ឋាន

| ទម្លាប់ | ហេតុអ្វី |
|--------|-----|
| branch អាយុខ្លី | រសាត់តិចពី `main` conflict តូចជាង |
| បើក PRs ញឹកញាប់ | Feedback ដើមមិនមែន 2,000-line surprise |
| ការពារ `main` | ការពិនិត្យឡើងវិញ + CI ទាមទារ; គ្មានការ push ដោយផ្ទាល់ |
...
- រារាំងការ push ដោយផ្ទាល់ទៅ `main`
...
កុំប្រព្រឹត្តអាថ៌កំបាំង ឬ `node_modules/`។ ការផ្លាស់ប្តូរឡូជីខលមួយក្នុងមួយ commit នៅពេលដែលអ្នកអាចធ្វើបាន។
...
- Stash គឺ local - វាមិនទៅ GitHub ទេ។
...
អតិថិជនដំណើរការ "កំណែ 1.4.2"។ ការដាក់ពង្រាយស្គ្រីបត្រូវការ**ឈ្មោះថេរ**សម្រាប់ commit មិនមែន “អ្វីក៏ដោយ `main` គឺថ្ងៃនេះ។
...
អ្នកស្ថិតនៅលើឈ្មោះ commit, **មិនមែន** នៅលើ branch។ commits ថ្មីអាចពិបាករក លុះត្រាតែអ្នកបង្កើត branch៖
